...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const ticker of tickers) {
      // One flaky ticker (price fetch or query hiccup) must not fail the whole
      // watchlist — degrade that row to a name-only/empty entry.
      try {
        ...
        console.error(`[watchlist] ${ticker} failed:`, e instanceof Error ? e.message : e);
        out.push({
          ticker,
          name: null,
          close: null,
    ...
    console.error("[watchlist] failed:", e instanceof Error ? e.message : e);
