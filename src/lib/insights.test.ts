...
// Fixture: officer I1 has three buys (+100%, +10%, −50% → median +10, 2/3 wins),
// director I2 has two big winners (+400% each) — enough for role stats, not
// enough for the leaderboard's 3-buy minimum.
let db: Db;
...
describe("overallOutcomes", () => {
  it("computes median (headline), mean, and win rate against latest closes", async () => {
    ...
    expect(stats!.buys).toBe(5);
    expect(stats!.tickers).toBe(5);
    // returns: +100, +10, −50, +400, +400 → median 100, mean 172
    expect(stats!.medianReturnPct).toBe(100);
    expect(stats!.avgReturnPct).toBe(172);
    expect(stats!.winRate).toBe(0.8);
  });
});

describe("topInsiders", () => {
  it("requires 3+ buys and ranks by median return", async () => {
    const leaders = await topInsiders(db);
    // I2's two +400% trades don't qualify — no two-lucky-trade heroes
    expect(leaders.map((l) => l.insider_cik)).toEqual(["I1"]);
    expect(leaders[0].median_ret).toBe("10.0");
    expect(leaders[0].buys).toBe("3");
  });
});
